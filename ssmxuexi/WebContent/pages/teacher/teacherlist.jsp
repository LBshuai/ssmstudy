...
<table   class="table table-hover">
 <tr> <th>
<form name="formsea" class="form-inline" role="form" method="post" action="<%=path %>/teacherlist">
  
  <div class="form-group">
    <div class="col-sm-2">
	<input type="text" name="name" id="name" class="form-control" placeholder="教师姓名"/>
	</div>
  </div>
...
<table class="table table-hover table-striped table-bordered">
 <tr><th>教师列表 &nbsp; &nbsp;
 <!--
 <button type="button" onclick="location.href='<%=path %>/teacheradd'" class="btn  btn-primary">添加</button>
 -->
</th></tr></table>
<table class="table table-hover table-striped table-bordered">
 <thead>
...
<td> 
 <a href="<%=path %>/teacheredit?id=${c.id}&forword=teacherlist" class="btn btn-default">修改</a> |
 <a href="teacherdel?id=${c.id}&forword=teacherlist" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
 </td>
</tr></tbody>
</c:forEach> </table><script type="text/javascript">
	       function go(index)
           {
               window.location.href="<%=path %>/teacherlist?index="+index;
           }
</script>
<table class="table table-hover table-striped table-bordered">
						        <tr>
