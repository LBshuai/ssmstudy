...
<table   class="table table-hover">
 <tr> <th>
<form name="formsea" class="form-inline" role="form" method="post" action="<%=path %>/banjilist">
  
  <div class="form-group">
    <div class="col-sm-2">
	<input type="text" name="name" id="name" class="form-control" placeholder="班级名称"/>
	</div>
  </div>
...
<table class="table table-hover table-striped table-bordered">
 <tr><th>班级列表 &nbsp; &nbsp;
 <!--
 <button type="button" onclick="location.href='<%=path %>/banjiadd'" class="btn  btn-primary">添加</button>
 -->
</th></tr></table>
<table class="table table-hover table-striped table-bordered">
 <thead>
...
<td> 
 <a href="<%=path %>/banjiedit?id=${c.id}&forword=banjilist" class="btn btn-default">修改</a> |
 <a href="banjidel?id=${c.id}&forword=banjilist" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
 </td>
</tr></tbody>
</c:forEach> </table><script type="text/javascript">
	       function go(index)
           {
               window.location.href="<%=path %>/banjilist?index="+index;
           }
</script>
<table class="table table-hover table-striped table-bordered">
						        <tr>
