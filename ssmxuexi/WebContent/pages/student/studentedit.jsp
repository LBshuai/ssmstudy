...
</script><form name="formAdd" method="post" action="studentupdate" class="form-horizontal" role="form">
...
<div class="form-group">
    <label class="col-sm-2 control-label" for="username">学号</label>
      <div class="col-sm-4">
<input name="username" type="text" id="username"  value="${c.username}"  class="form-control" >
 </div>
	</div>
...
<div class="form-group">
 <label class="col-sm-2 control-label" for="ds_password">性别</label>
   ...
   <option value="${s.name}"<c:if test="${s.name eq c.banji}"> selected</c:if>>${s.name}</option>
</c:forEach>
		 </select> 
		 </div>
   </div>
